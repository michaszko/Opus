...
char fileName[100]="mecEvents.root";
...
void makeMatrix()
{
		...
		if (e->fof(211, -211, 111) == 0 && e->fof(321, -321, 311) == 0 && e->fof(-311, 130, 310) == 0){ // no pions
		  ...
		  if (kos > kosmin) {
		    ...
		    if (muonlong >= 1500.00 && muonlong <= 20000.00) {
				x = position(muonlong, long_anu_left, longbins_anu);
				y = position(muontrans, trans_anu_left, transbins_anu);

	      		xx = int(entrans/1200.0 * dim);
				yy = int(sqrt(momtrans2)/1200.0 * dim);

				// row = y + 13 * x ;
				row = y * longbins_anu + x;
				column = xx * dim + yy;
	...
	for (int i = 0; i < longbins_anu*transbins_anu; ++i){
