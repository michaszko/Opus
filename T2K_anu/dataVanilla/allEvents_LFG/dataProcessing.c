...
ofstream nuwro2("../../data/cros_total_nuwro_LFG.dat");
...
void asia_a(TFile *Input, int ilezdarzen,
  ...
  TH2D *h1qel =
      new TH2D("h1qel", "h1qel", mombins, mom_left, cosbins, cos_left);
  TH2D *h1res =
      new TH2D("h1res", "h1res", mombins, mom_left, cosbins, cos_left);
  TH2D *h1mec =
      new TH2D("h1mec", "h1mec", mombins, mom_left, cosbins, cos_left);
  TH2D *h1dis =
      new TH2D("h1dis", "h1dis", mombins, mom_left, cosbins, cos_left);
  ...
  delete h1, h1qel, h1res, h1mec, h1dis;
}
...
////////////////////////////////////////
{
  ...
  TFile *tf1 = new TFile(fileNameROOT);
  if (!tf1->IsOpen())
    cout << "Huston, we have a problem with loading file :| " << endl;
  else
  ...
  for (int j = 0; j < totalbins; j++) {
    ...
    cross[j] = sumka / events * cross_t2k /
               (cos_left[koss + 1] - cos_left[koss]) /
               (mom_left[momm + dlugosc] - mom_left[momm]) * 1e3;
    cross_qel[j] = sumka_qel / events * cross_t2k /
                   (cos_left[koss + 1] - cos_left[koss]) /
                   (mom_left[momm + dlugosc] - mom_left[momm]) * 1e3;
    cross_res[j] = sumka_res / events * cross_t2k /
                   (cos_left[koss + 1] - cos_left[koss]) /
                   (mom_left[momm + dlugosc] - mom_left[momm]) * 1e3;
    cross_mec[j] = sumka_mec / events * cross_t2k /
                   (cos_left[koss + 1] - cos_left[koss]) /
                   (mom_left[momm + dlugosc] - mom_left[momm]) * 1e3;
    cross_dis[j] = sumka_dis / events * cross_t2k /
                   (cos_left[koss + 1] - cos_left[koss]) /
                   (mom_left[momm + dlugosc] - mom_left[momm]) * 1e3;
  ...
  //     error[s] = // sqrt( //ciro_nu_cov_norm [s*(totalbins+1)]+
  //     ciro_nu_cov_shape
  //         // [s*(totalbins+1)] +
  //         ciro_nu_errors[s];
